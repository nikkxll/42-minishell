...
/*   Updated: 2024/03/14 22:37:07 by dnikifor         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	entities_expand(char ***temp_arr, char *str)
{
	...
	while (entry != NULL)
	{
		if (check_entity(entry->d_name, str, last, NULL))
		{
...
int	wildcards(char ***arr)
{
	...
	arr_len = ft_arrlen((void **)aux_arr);
	...
	while (i < arr_len)
	{
		...
		if (!temp_arr[i++])
		{
			ft_free_3d_array(temp_arr);
			return (MALLOC_ERR);
		}
	}
	i = -1;
	while (++i < arr_len)
		entities_expand(&(temp_arr[i]), aux_arr[i]);
	i = -1;
	arr_len = 0;
